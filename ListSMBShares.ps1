...
param(
    ...
    [int]$Throttle = 20,    # Maximum number of concurrent remote connections

    [string]$ServerListPath,        # The Server List, on which the scripts runs. If it is empty the Script will search for Server in the AD
...
function Test-Remoting {
    ...
    try {
        ...
        $res = Test-WSMan -ComputerName $ComputerName -Authentication Default -ErrorAction Stop
        $sw.Stop()
        return $true
    } catch {
        Write-Warning "WSMan not reachable on ${ComputerName}: $($_.Exception.Message)"
        return $false
    }
}
...
function Get-TargetServers {
    ...
    if ($Path) {
        if (-not (Test-Path $Path)) { throw "ServerListPath not found: $Path" }
        $list += Get-Content -Path $Path | Where-Object { $_ -and $_.Trim() -ne '' } | ForEach-Object { $_.Trim() }
    }
    if (-not $list) { throw "No Server found. Use -ServerListPath." }
    $list | Sort-Object -Unique
}

if(-not $ServerListPath){
    Write-Host "Use Active Directory to find Server" -ForegroundColor Cyan

    # Get all Server from Active Directory with the DNS Host Name
    $adServers = Get-ADComputer -Filter * -Properties OperatingSystem, DNSHostName |
        Where-Object {
            $_.OperatingSystem -like "*Server*" -and
            $_.OperatingSystem -notmatch "Linux|Ubuntu|Debian|Red Hat|SUSE"
        } | Select-Object -ExpandProperty DNSHostName

    Write-Host "Servers in AD: $($adServers.Count)" -ForegroundColor Cyan

    # Test Reachability for Servers
    $reachableServers = foreach ($srv in $adServers) {
        Write-Host "Test WinRM on $srv ..." -ForegroundColor DarkCyan
        if (Test-WSMan $srv -ErrorAction SilentlyContinue) {
            Write-Host "Server $srv reachable" -ForegroundColor DarkCyan
            $srv
        } else {
            Write-Warning "$srv NOT reachable."
        }
    }

    Write-Host "`nWinRM-reachable Servers: $($reachableServers.Count)" -ForegroundColor Cyan

    # Remove local Host from List
    $localHostName = "$env:COMPUTERNAME".ToUpper()

    $reachableServers = $reachableServers | Where-Object {
        $remoteShort = $_.Split('.')[0].ToUpper()
        $remoteShort -ne $localHostName
    }
}else{
    Write-Host "Use text file to find Server" -ForegroundColor Cyan

    $targets = Get-TargetServers -Inline $Servers -Path $ServerListPath

    Write-Host "Found Server: $($targets -join ', ')" -ForegroundColor Cyan

    $reachableServers = foreach ($srv in $targets) {
        if (Test-Remoting -ComputerName $srv) { $srv }
    }
}
...
# Running Command Block on all remote Computers in AD
if($reachableServers.Count -gt 0){
    $remoteResult = Invoke-Command -ComputerName $reachableServers `
        -ScriptBlock $sbSummary `
        -ThrottleLimit $Throttle `
        -ArgumentList $IncludeAdminShares.IsPresent, $WithNfsShares.IsPresent

    $summary +=$remoteResult
}
